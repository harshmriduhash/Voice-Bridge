...
const ListeningDashboard = () => {
    ...
    const loadAnnouncementData = () => {
      try {
        ...
        if (location.state?.announcement) {
          ...
          const storedAnnouncement = localStorage.getItem(
            "currentAnnouncement"
          );
  ...
  const getTranslations = () => {
    ...
    return Object.entries(currentAnnouncement.translations).map(
      ([code, text]) => ({
  ...
  const playAudio = async (languageCode, languageName) => {
    try {
      ...
      audio.onerror = (error) => {
        ...
        audioRef.current = null;
        alert(
          `Error playing audio for ${languageName}. Please check the audio file.`
        );
      };
  ...
  useEffect(() => {
    ...
    const demoText =
      ...
      "Welcome to VoiceBridge. Your announcement will appear here once processed.";
                    ...
                    <details open={isTranslationsOpen} className="group">
                      ...
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {translations.map((translation, index) => (
                          <div
                            key={index}
                            className="bg-subtle-light/50 dark:bg-subtle-dark/50 p-4 rounded-xl border"
                          >
