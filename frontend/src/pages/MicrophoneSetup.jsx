...
const MicrophoneSetup = () => {
  ...
  const setupMediaRecorder = (stream) => {
    try {
      ...
      console.log(
        "MediaRecorder setup complete with mimeType:",
        options.mimeType
      );
    } catch (error) {
  ...
  const startRecording = () => {
    ...
    if (selectedLanguages.length === 0) {
      setError(
        "No languages selected. Please go back and select languages first."
      );
      return;
    }
    ...
    if (mediaRecorderRef.current && microphoneStatus === "connected") {
      ...
      console.log(
        "Started recording audio. Selected languages:",
        selectedLanguages
      );
    }
  };
  ...
  const sendAudioToBackend = async () => {
    try {
      ...
      setRecordingStatus(
        "Audio processed successfully! Redirecting to dashboard..."
      );
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 lg:p-8">
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-lg text-text-light mb-8">
            Allow VoiceBridge to use your device's microphone to capture
            announcements.
          </p>
            ...
            <div
              className={`mb-4 p-3 rounded-lg border ${
                ...
                recordingStatus.includes("error")
                  ...
                  : recordingStatus.includes("Processing") ||
                    recordingStatus.includes("Sending")
          ...
          <div className="mt-8 space-y-4">
            ...
            <div
              className={`flex items-center p-4 rounded-xl border transition-all duration-300 ${
                ["connected", "recording", "processing"].includes(
                  microphoneStatus
                )
            ...
            >
              <div
                className={`flex-shrink-0 rounded-full p-2 mr-4 transition-all duration-300 ${
                  ["connected", "recording", "processing"].includes(
                    microphoneStatus
                  )
